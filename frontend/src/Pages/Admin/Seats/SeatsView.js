...
import { SeatButton, SeatCellContainer, SeatInfo, SeatsTable } from './Seats.style';
    ...
    return (
      <FormSelect>
        {props.halls.map((item, index) => {
          return (
            <SelectOption key={shortid.generate()} onClick={() => selectHall(item)}>
              {item.name}
            </SelectOption>
          );
        })}
  ...
  return (
    <>
      <VerticalContainer>
        ...
        {generateSelector()}
        <ManageOption onClick={() => toggleDisplayForm()}>Add Seat</ManageOption>
        {generateForm()}
        {generateSeatsGrid()}
      </VerticalContainer>
    </>
