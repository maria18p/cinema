...
presentation_seat_route.post('/getByPresentation', async (req, res) => {
  ...
  const result = await get_request_presentation_seat_by_presentation_id(requestObject);
  res.send(await get_request_presentation_seat_by_presentation_id(requestObject));
});
